...
const checkParamsForStoredProcedures = (params: any[]) => {
    return params.map((param) => {
        if (
            typeof param === 'object' &&
            instanceOfyupFormStoredProcedure(param)
        ) {
            return storedProcedures(
                param.functionName,
                param.functionArguements
            );
        }
        return param;
    });
    ...
    if (otherwise) {
        whenParams.otherwise = (schema: any) => {
            ...
            otherwise?.map((otherwiseParams: validationRequirementProps) => {
                let tmpValidator = yupArgCreator(
                    otherwiseParams,
                    schema,
                    parentType
                );
...
const ArrayOfObjectsGeneration = (of: yupFormProperties | undefined) => {
    //yup array of objects
    let yupArray = yup.array();

    if (of?.type == 'object' && of?.shape) {
        ...
        for (const shapeKey in shape) {
            let shapeType = (
                yup[shape[shapeKey].type as keyof typeof yup] as Function
            )();

            //validations can be a string or an object
            shape[shapeKey].validations?.forEach((validation) => {
            ...
            yupObject = yupObject.shape({ [shapeKey]: shapeType });
        }

        yupArray = yup.array<any>().of(yupObject) as any; //typescript doesn't like the dynamic key
    }
...
export const yupGeneration = (schema: yupFormBuilderProps) => {
    ...
    try {
        ...
        for (const key in properties) {
            ...
            const { type, validations, of } = properties[key];
            ...
            if (type == 'array' && of) {
                (yupSchema[key as keyof typeof yupSchema] as any) =
